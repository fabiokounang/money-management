/**
 * Adds transactions.include_in_budget if missing.
 * Usage: node scripts/run-migration-transaction-budget-flag.js
 */
...
async function main() {
  ...
  try {
    const exists = await hasColumn(conn, database, 'transactions', 'include_in_budget');
    if (!exists) {
      await conn.query(
        'ALTER TABLE transactions ADD COLUMN include_in_budget TINYINT(1) NOT NULL DEFAULT 1 AFTER include_in_dashboard'
      );
    }
    console.log('Migration transaction include_in_budget: OK');
  } finally {
    await conn.end();
  }
}
